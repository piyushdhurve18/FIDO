...
#failed case 12
def rpIdDataNotText(pin, username, display, rp):
    hashchallenge = os.urandom(32)
    util.printcolor(util.YELLOW,"")
    util.printcolor(util.YELLOW, "****MakeCredential Request 2****")
    ...
    result = rpIdNotText(pin, hashchallenge, rp, username)

def rpIdNotText(curpin, clientDataHash, rp, user):
    util.printcolor(util.YELLOW, f"Using PIN data: {curpin}")
    util.APDUhex("00a4040008a0000006472f0001", "Select applet")
    ...
    pinAuthToken = util.hmac_sha256(pinToken, clientDataHash)
    util.printcolor(util.CYAN, f"pinAuthToken: {pinAuthToken.hex()}")
    makeCredAPDU = rpIdNotTypeText(clientDataHash, rp, user, pubkey, pinAuthToken);
    last_response = None
    last_status = None
...
def rpIdNotTypeText(clientDataHash, rp, user, credParam, pinAuthToken):
    PublicKeyCredentialUserEntity = {
        "id": user.encode(),
        "name": user,
        "displayName": user
    ...
    # "rp.id" is NOT a text string (invalid for F-12)
    PublicKeyCredentialRpEntity = {
        "id": 12353,
        "name":rp
    }
...
#failed case 13
def rpNameDataNotText(pin, username, display, rp):
    hashchallenge = os.urandom(32)
    util.printcolor(util.YELLOW,"")
    util.printcolor(util.YELLOW, "****MakeCredential Request 2****")
    ...
    result = rpNameNotText(pin, hashchallenge, rp, username)


def rpNameNotText(curpin, clientDataHash, rp, user):
    util.printcolor(util.YELLOW, f"Using PIN data: {curpin}")
    util.APDUhex("00a4040008a0000006472f0001", "Select applet")
    ...
    pinAuthToken = util.hmac_sha256(pinToken, clientDataHash)
    util.printcolor(util.CYAN, f"pinAuthToken: {pinAuthToken.hex()}")
    makeCredAPDU = rpNameNotTypeText(clientDataHash, rp, user, pubkey, pinAuthToken);
    last_response = None
    last_status = None
    for apdu in makeCredAPDU:
        response, status = util.APDUhex(apdu, None, checkflag=False)
...
def rpNameNotTypeText(clientDataHash, rp, user, credParam, pinAuthToken):
    PublicKeyCredentialUserEntity = {
        "id": user.encode(),
        "name": user,
        "displayName": user
...
#failed case 14
def rpIconDataNotText(pin, username, display, rp):
    hashchallenge = os.urandom(32)
    util.printcolor(util.YELLOW,"")
    util.printcolor(util.YELLOW, "****MakeCredential Request 2****")
    ...
    result = rpIconNotText(pin, hashchallenge, rp, username)


def rpIconNotText(curpin, clientDataHash, rp, user):
    util.printcolor(util.YELLOW, f"Using PIN data: {curpin}")
    util.APDUhex("00a4040008a0000006472f0001", "Select applet")
    ...
    pinAuthToken = util.hmac_sha256(pinToken, clientDataHash)
    util.printcolor(util.CYAN, f"pinAuthToken: {pinAuthToken.hex()}")
    makeCredAPDU = rpIconNotTypeText(clientDataHash, rp, user, pubkey, pinAuthToken);
    last_response = None
    last_status = None
    for apdu in makeCredAPDU:
        response, status = util.APDUhex(apdu, None, checkflag=False)
...
def rpIconNotTypeText(clientDataHash, rp, user, credParam, pinAuthToken):
    PublicKeyCredentialUserEntity = {
        "id": user.encode(),
        "name": user,
        "displayName": user
...
#failed case 15
def userIdDataNotText(pin, username, display, rp):
    hashchallenge = os.urandom(32)
    util.printcolor(util.YELLOW,"")
    util.printcolor(util.YELLOW, "****MakeCredential Request 3****")
    util.printcolor(util.YELLOW, """TTest started: F-1
        Send CTAP2 authenticatorMakeCredential(0x01) message, with "user.id" is NOT of type BYTE ARRAY, wait for the response, and check that Authenticator returns an error.""")
    result = userIdNotText(pin, hashchallenge, rp, username)


def userIdNotText(curpin, clientDataHash, rp, user):
    util.printcolor(util.YELLOW, f"Using PIN data: {curpin}")
    util.APDUhex("00a4040008a0000006472f0001", "Select applet")
    ...
    pinAuthToken = util.hmac_sha256(pinToken, clientDataHash)
    util.printcolor(util.CYAN, f"pinAuthToken: {pinAuthToken.hex()}")
    makeCredAPDU = userIdNotTypeText(clientDataHash, rp, user, pubkey, pinAuthToken);
    last_response = None
    last_status = None
    for apdu in makeCredAPDU:
        response, status = util.APDUhex(apdu, None, checkflag=False)
...
def userIdNotTypeText(clientDataHash, rp, user, credParam, pinAuthToken):
    PublicKeyCredentialUserEntity = {
        "id":[],
        "name": user,
        "displayName": user
...
#failed case 16
def userNameDataNotText(pin, username, display, rp):
    hashchallenge = os.urandom(32)
    util.printcolor(util.YELLOW,"")
    util.printcolor(util.YELLOW, "****MakeCredential Request 3****")
    util.printcolor(util.YELLOW, """Test started: F-2
        Send CTAP2 authenticatorMakeCredential(0x01) message, with "user.name" is NOT of type TEXT, wait for the response, and check that Authenticator returns an error.""")
    result = userNameNotText(pin, hashchallenge, rp, username)


def userNameNotText(curpin, clientDataHash, rp, user):
    util.printcolor(util.YELLOW, f"Using PIN data: {curpin}")
    util.APDUhex("00a4040008a0000006472f0001", "Select applet")
    ...
    pinAuthToken = util.hmac_sha256(pinToken, clientDataHash)
    util.printcolor(util.CYAN, f"pinAuthToken: {pinAuthToken.hex()}")
    makeCredAPDU = userNameNotTypeText(clientDataHash, rp, user, pubkey, pinAuthToken);
    last_response = None
    last_status = None
    for apdu in makeCredAPDU:
        response, status = util.APDUhex(apdu, None, checkflag=False)
...
def userNameNotTypeText(clientDataHash, rp, user, credParam, pinAuthToken):
    PublicKeyCredentialUserEntity = {
        "id":user.encode(),
        "name":True,#user name not tye text
        "displayName": user
    }
...
#failed case 17
def userDisplayDataNotText(pin, username, display, rp):
    hashchallenge = os.urandom(32)
    util.printcolor(util.YELLOW,"")
    util.printcolor(util.YELLOW, "****MakeCredential Request 3****")
    util.printcolor(util.YELLOW, """Test started: F-3
        Send CTAP2 authenticatorMakeCredential(0x01) message, with "user.displayName" is NOT of type TEXT, wait for the response, and check that Authenticator returns an error.""")
    result = userDisplayNotText(pin, hashchallenge, rp, username)


def userDisplayNotText(curpin, clientDataHash, rp, user):
    util.printcolor(util.YELLOW, f"Using PIN data: {curpin}")
    util.APDUhex("00a4040008a0000006472f0001", "Select applet")
    ...
    pinAuthToken = util.hmac_sha256(pinToken, clientDataHash)
    util.printcolor(util.CYAN, f"pinAuthToken: {pinAuthToken.hex()}")
    makeCredAPDU = userDisplayNotTypeText(clientDataHash, rp, user, pubkey, pinAuthToken);
    last_response = None
    last_status = None
    for apdu in makeCredAPDU:
        response, status = util.APDUhex(apdu, None, checkflag=False)
...
def userDisplayNotTypeText(clientDataHash, rp, user, credParam, pinAuthToken):
    PublicKeyCredentialUserEntity = {
        "id":user.encode(),
        "name":user,
        "displayName": 7832
...
#failed case 18
def userIconDataNotText(pin, username, display, rp):
    hashchallenge = os.urandom(32)
    util.printcolor(util.YELLOW,"")
    util.printcolor(util.YELLOW, "****MakeCredential Request 3****")
    util.printcolor(util.YELLOW, """TTest started: F-4
        Send CTAP2 authenticatorMakeCredential(0x01) message, with "user.icon" is NOT of type TEXT, wait for the response, and check that Authenticator returns an error.""")
    result = userIconNotText(pin, hashchallenge, rp, username)


def userIconNotText(curpin, clientDataHash, rp, user):
    util.printcolor(util.YELLOW, f"Using PIN data: {curpin}")
    util.APDUhex("00a4040008a0000006472f0001", "Select applet")
    ...
    pinAuthToken = util.hmac_sha256(pinToken, clientDataHash)
    util.printcolor(util.CYAN, f"pinAuthToken: {pinAuthToken.hex()}")
    makeCredAPDU = userIconNotTypeText(clientDataHash, rp, user, pubkey, pinAuthToken);
    last_response = None
    last_status = None
    for apdu in makeCredAPDU:
        response, status = util.APDUhex(apdu, None, checkflag=False)
...
def userIconNotTypeText(clientDataHash, rp, user, credParam, pinAuthToken):
    PublicKeyCredentialUserEntity = {
        "id":user.encode(),
        "name":user,
        "displayName": user,
...
# ---- Test Case Descriptions ----
    ...
    Send CTAP2 authenticatorMakeCredential(0x01) message, with "rp.name" is NOT of type TEXT, wait for the response, and check that Authenticator returns an error.""")
     ...
     Send CTAP2 authenticatorMakeCredential(0x01) message, with "rp.icon" is NOT of type TEXT, wait for the response, and check that Authenticator returns an error.""")
        ...
        raise ValueError("Invalid mode! Use: F12 | F13 | F14")
